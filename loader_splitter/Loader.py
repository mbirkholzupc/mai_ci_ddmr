...
class Loader:
    ...
    def load(self) -> SupervisedData:
        ...
        samples = np.empty((n_images, self._image_size[0], self._image_size[1], 3), dtype=np.uint8)
        labels = []
        image_index = 0
        for i, category in enumerate(self._categories):
            ...
            for j, filename in enumerate(filenames):
                ...
                samples[image_index] = resize(imread(image_path).astype(np.uint8), self._image_size,
                                              anti_aliasing=True)
